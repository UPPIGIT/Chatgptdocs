Let's adjust the CSS styles to ensure that the table borders are visible and the email looks good. I'll make sure the borders are defined clearly.

### Updated `EmailUtility` Class with Improved Table Borders
...
1. **Table Borders**: Added `border: 1px solid #ddd;` to the table, th, and td elements to ensure that the table borders are visible.
2. **Improved Styling**: Maintained a consistent color scheme and added styles for table rows to alternate background colors for better readability.

This should ensure that the tables in your email have clearly visible borders and look good in most email clients.
